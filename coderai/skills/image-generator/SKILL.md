...
积分试算允许匿名调用，不需要配置 Key。生成图片时，脚本从 `~/.coderai-plus/settings.json` 读取 `env.PLUS_API_KEY`：
...
让用户从 [Deep Code Plus 平台](https://coderai.vegamo.cn/plus/api-keys) 获取 Key 并在本地写入该文件。不要要求用户在聊天中发送 Key，也不要在输出中显示 Key。
