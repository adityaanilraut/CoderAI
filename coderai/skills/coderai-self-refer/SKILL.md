...
description: Answers questions about CoderAI CLI itself — including features, configuration options, slash commands, Skills, MCP integration, permissions, notifications, session persistence, and troubleshooting. Use this when users ask how to configure or use CoderAI, how to set up an MCP server, configure notifications (such as Slack/Feishu), manage permissions, view available skills, understand slash commands, configure thinking mode, etc.
---

# CoderAI Self-Refer

This Skill helps you answer user questions about CoderAI CLI itself by consulting the reference documentation bundled with this Skill. All docs live in the `references/` subdirectory — always refer to them for authoritative answers.

## When to use this Skill

Use this Skill when the user asks any question about CoderAI itself, such as:
...
- "会话历史保存在哪里？"
- "/undo 是怎么工作的？"
- "CoderAI 和 VSCode 插件怎么配合？"
- Any other question about CoderAI CLI's features, configuration, or usage.
...
- If answering directly, do not infer the list only from loaded skill prompts or from project/user directories. Enumerate all discovery roots:
  1. `./.coderai/skills/<folder>/SKILL.md`
  2. `./.agents/skills/<folder>/SKILL.md`
  3. `~/.coderai/skills/<folder>/SKILL.md`
...
- Clearly separate discoverable skills from other concepts:
  ...
  - Bundled skills are discoverable skills shipped with CoderAI, such as `bundled:coderai-self-refer/SKILL.md`.
...
- Clarify user-level (`~/.coderai/settings.json`) vs project-level (`.coderai/settings.json`)
...
3. **Be specific about file paths** — always specify whether it's `~/.coderai/settings.json` or `.coderai/settings.json`.
...
- Skills are discovered from: `./.coderai/skills/`, `./.agents/skills/`, `~/.coderai/skills/`, `~/.agents/skills/`, and bundled built-in skills such as `bundled:coderai-self-refer/SKILL.md`.
- In a source checkout, check `templates/skills/bundled/*/SKILL.md`; in a packaged install, check `dist/bundled/*/SKILL.md`.
- Built-in bundled skills may include `coderai-self-refer`, `plan`, `skill-digester`, and `skill-writer`; verify the actual list by scanning the bundled root because it can change between versions.
- Use `/skills` slash command in the CoderAI CLI to list all available skills
...
- Add to `settings.json` (user-level `~/.coderai/settings.json` or project-level `.coderai/settings.json`):
...
- After saving, use `/mcp` in CoderAI to verify the server is running
