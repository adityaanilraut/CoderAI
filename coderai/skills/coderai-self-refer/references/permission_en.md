...
Configure permissions in `~/.coderai/settings.json` (user-level) or `.coderai/settings.json` (project-level) via the `permissions` field:
...
When you select "Yes, and always allow" in a permission prompt, the corresponding scope is written to the project's `.coderai/settings.json`:
