...
编辑 `~/.coderai/settings.json`，添加 `notify` 字段：
...
#!/bin/bash
# macOS 系统通知
osascript -e "display notification \"任务已${STATUS:-完成}，耗时 ${DURATION}s\" with title \"CoderAI\""
```
...
notify-send "CoderAI" "任务已${STATUS:-完成}，耗时 ${DURATION}s"
```
