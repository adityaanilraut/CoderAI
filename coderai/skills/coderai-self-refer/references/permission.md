...
在 `~/.coderai/settings.json`（用户级）或 `.coderai/settings.json`（项目级）中通过 `permissions` 字段配置：
...
当用户在权限提示中选择 "Yes, and always allow" 后，对应的权限范围会被写入当前项目的 `.coderai/settings.json` 文件中：
