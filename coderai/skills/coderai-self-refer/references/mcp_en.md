...
Edit `~/.coderai/settings.json` and add the `mcpServers` field:
...
Below is a complete `~/.coderai/settings.json` with both GitHub and Playwright MCP servers configured:
...
After configuration, start `coderai` and type `/mcp` in the chat to view the status of all configured MCP servers and the list of tools each server provides.

Simply use the MCP tool name in your conversation to invoke it, for example:

```
Help me search for issues in the coderai-cli repository on GitHub
```
...
3. Whether the terminal running `coderai` has network access

### Tools Not Showing Up

1. Verify that the `mcpServers` field in `settings.json` is correctly formatted
2. After starting coderai, use `/mcp` to check server status
