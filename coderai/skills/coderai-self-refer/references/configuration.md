...
| 文件类型     | 位置                                 | 作用范围                                                        |
| ------------ | ------------------------------------ | --------------------------------------------------------------- |
| 用户设置文件 | `~/.coderai/settings.json`          | 适用于当前用户的所有 Deep Code 会话。                           |
| 项目设置文件 | `项目根目录/.coderai/settings.json` | 仅在该特定项目中运行 Deep Code 时生效。项目设置会覆盖用户设置。 |
...
1. settings.json 外层的 env：这是针对整个工具及其所有子进程的通用配置（全局变量）。可被外层环境变量覆盖，但环境变量KEY会移除`CODERAI_`前缀。
...
4. 系统环境变量: `CODERAI_API_KEY=abc123 coderai`
...
6. 系统环境变量: `CODERAI_THINKING_ENABLED=true coderai`
...
4. 系统环境变量: `CODERAI_WEBHOOK=... coderai`
...
5. 系统环境变量: `CODERAI_MCP_GITHUB_PERSONAL_ACCESS_TOKEN=... coderai`
