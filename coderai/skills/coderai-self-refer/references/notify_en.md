...
Edit `~/.coderai/settings.json` and add the `notify` field:
...
```json
{
  ...
  "notify": "/Users/you/.coderai/notify-slack.sh"
}
```
...
Create `~/.coderai/notify-slack.sh`:
...
```bash
chmod +x ~/.coderai/notify-slack.sh
```
...
process.stdout.write(JSON.stringify({
  ...
  card: {
    header: { title: { tag: 'plain_text', content: 'CoderAI: ' + process.env.TITLE + ' ' + process.env.STATUS + ' [' + process.env.DURATION + 's]' } },
...
```json
{
  ...
  "notify": "/Users/you/.coderai/notify-feishu.sh"
}
```
...
Create `~/.coderai/notify.sh`:

```bash
#!/bin/bash
# iTerm2 / Windows Terminal OSC 9 notification
printf '\x1b]9;CoderAI: task %s (%ss)\x07' "${STATUS:-completed}" "${DURATION}"
```

```json
{
  "notify": "/Users/you/.coderai/notify.sh"
}
```
...
echo \x1b]9;CoderAI: task %STATUS% (%DURATION%s)\x07
```
...
#!/bin/bash
# macOS system notification
osascript -e "display notification \"Task ${STATUS:-completed}, took ${DURATION}s\" with title \"CoderAI\""
```
...
notify-send "CoderAI" "Task ${STATUS:-completed}, took ${DURATION}s"
```

```json
{
  "notify": "/home/you/.coderai/notify.sh"
}
```
...
msg %USERNAME% "CoderAI: task %STATUS% (%DURATION%s)"
```

```json
{
  "notify": "C:\\Users\\you\\.coderai\\notify.bat"
}
```
