...
if TYPE_CHECKING:
    from coderai.soul.kimisoul import SessionSoul

type SoulSlashCmdFunc = Callable[[SessionSoul, str], None | Awaitable[None]]
"""
A function that runs as a SessionSoul-level slash command.
...
@registry.command
async def init(soul: SessionSoul, args: str):
    """Analyze the codebase and generate an `AGENTS.md` file"""
    from coderai.soul.kimisoul import SessionSoul

    with tempfile.TemporaryDirectory() as temp_dir:
        tmp_context = Context(file_backend=Path(temp_dir) / "context.jsonl")
        tmp_soul = SessionSoul(soul.agent, context=tmp_context)
...
@registry.command
async def compact(soul: SessionSoul, args: str):
...
async def clear(soul: SessionSoul, args: str):
    """Clear the context"""
...
@registry.command
async def yolo(soul: SessionSoul, args: str):
...
@registry.command
async def afk(soul: SessionSoul, args: str):
...
@registry.command
async def plan(soul: SessionSoul, args: str):
...
async def add_dir(soul: SessionSoul, args: str):
    """Add a directory to the workspace. Usage: /add-dir <path>. Run without args to list added dirs"""  # noqa: E501
    from kaos.path import KaosPath

    from coderai.utils.path import is_within_directory, list_directory
...
@registry.command
async def export(soul: SessionSoul, args: str):
    """Export current session context to a markdown file"""
    from coderai.utils.export import perform_export
...
async def import_context(soul: SessionSoul, args: str):
    """Import context from a file or session ID"""
    from coderai.utils.export import perform_import
