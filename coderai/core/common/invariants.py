...
class InvariantViolation(Exception):
    ...
    def __init__(self, violations: list[str]) -> None:
        ...
        super().__init__(f"Runtime invariant violations ({len(violations)}): " + "; ".join(violations))
...
def verify_monotonic_sequence_numbers(events: list[Any]) -> list[str]:
    ...
    for idx, ev in enumerate(events):
        seq = getattr(ev, "seq", None) if hasattr(ev, "seq") else (ev.get("seq") if isinstance(ev, dict) else None)
        if seq is None:
            continue
        if not isinstance(seq, int) or seq <= 0:
            violations.append(f"Event at index {idx} has invalid non-positive sequence number: {seq}")
            continue
        if last_seq is not None:
            if seq <= last_seq:
                violations.append(f"Event at index {idx} has non-monotonic sequence number: {seq} <= {last_seq}")
        last_seq = seq
...
def verify_paired_tool_calls(items: list[Any]) -> list[str]:
    ...
    for idx, item in enumerate(items):
        role = getattr(item, "role", None) if hasattr(item, "role") else (item.get("role") if isinstance(item, dict) else None)
        ev_type = getattr(item, "type", None) if hasattr(item, "type") else (item.get("type") if isinstance(item, dict) else None)

        if role == "assistant" or ev_type == "tool/call":
            # Extract tool calls
            tool_calls = getattr(item, "tool_calls", None) if hasattr(item, "tool_calls") else (item.get("tool_calls") if isinstance(item, dict) else None)
            ...
            tc_id = getattr(item, "tool_call_id", None) if hasattr(item, "tool_call_id") else (item.get("tool_call_id") if isinstance(item, dict) else (item.get("toolCallId") if isinstance(item, dict) else None))
...
def verify_turn_step_boundaries(events: list[Any]) -> list[str]:
    ...
    for idx, ev in enumerate(events):
        ev_type = getattr(ev, "type", None) if hasattr(ev, "type") else (ev.get("type") if isinstance(ev, dict) else None)
        if not ev_type:
            continue

        if ev_type == "turn/start":
            if in_turn:
                violations.append(f"Nested turn/start at index {idx} while previous turn is still open")
            in_turn = True
            ...
            if in_step:
                violations.append(f"Nested step/start at index {idx} while previous step is still open")
            in_step = True
