...
class ApprovalRequestPanel:
            ...
            if isinstance(block, DiffDisplayBlock):
                ...
                if non_diff_budget > 0:
                    ...
                    if line_count > non_diff_budget:
                        truncated = "\n".join(text.split("\n")[:non_diff_budget])
                        self._non_diff_truncated = True
                    self._preview_renderables.append(CoderAISyntax(truncated, block.language))
                    non_diff_budget -= min(line_count, non_diff_budget)
                else:
                    self._non_diff_truncated = True
                idx += 1
    ...
    def _render_block(
        ...
        if block.lexer:
            return CoderAISyntax(text, block.lexer)
    ...
    with console.screen(), console.pager(styles=True):
        ...
        while idx < len(display):
            ...
            if isinstance(block, DiffDisplayBlock):
                ...
                console.print(CoderAISyntax(block.command.rstrip("\n"), block.language))
                rendered_any = True
                idx += 1
            elif isinstance(block, BriefDisplayBlock) and block.text:
...
def extract_plan_options(plan_text: str) -> list[dict[str, str]]:
    """Return [{key, title}] for ``## Option X`` sections (max 3)."""
    options: list[dict[str, str]] = []
    for m in _OPTION_RE.finditer(plan_text or ""):
        key = m.group(1).upper()
        title = (m.group(2) or "").strip()[:80]
        if key not in [o["key"] for o in options]:
            options.append({"key": key, "title": title or f"Option {key}"})
        if len(options) >= 3:
            break
    return options


def is_recommended(title: str) -> bool:
    return bool(_RECOMMENDED_RE.search(title or ""))
...
def prompt_plan_review(
    ...
    action: approve | option | revise | reject | reject-exit
    """
    from coderai.ui.shell.session_picker import select_with_arrows

    choose = select_fn or (lambda c, items, **kw: select_with_arrows(c, items, **kw))
    options = extract_plan_options(plan_text)
    ...
    items += [
        ("revise", "Revise plan", "Give feedback; the agent updates and resubmits"),
        ("reject", "Reject", "Stay in plan mode with feedback via conversation"),
    ...
    key = items[res][0] if isinstance(res, int) and 0 <= res < len(items) else str(res or "reject")
    if key == "approve":
        return {"action": "approve"}
    if key.startswith("option-"):
        return {"action": "option", "option": key.split("-", 1)[1]}
    if key == "revise":
        try:
            fb = input("Revision notes: ").strip()
        except (EOFError, KeyboardInterrupt):
            fb = ""
        return {"action": "revise", "feedback": fb}
    if key == "reject-exit":
        return {"action": "reject-exit"}
    return {"action": "reject"}


def expand_plan_in_pager(plan_text: str) -> None:
    """Ctrl-E parity: fullscreen pager for the full plan."""
    try:
        import pydoc
