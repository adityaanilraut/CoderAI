...
def run_quick_setup_wizard(
    ...
    provider_options = [
        (
            "oauth",
            "Log in with browser (OAuth device flow)",
            "No API key needed — authorizes via kimi-code / managed provider",
        ),
        (
            "openai",
    ...
    if chosen_provider_tag == "oauth":
        from coderai.ui.shell.oauth import perform_login_sync

        if perform_login_sync(open_browser=True) != 0:
            if console is not None and _RICH:
                console.print("  [dim]OAuth login did not complete.[/dim]\n")
            return
        clear_client_pool()
        try:
