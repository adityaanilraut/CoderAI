...
call_soon_threadsafe via asyncio loop. Windows path via msvcrt.
"""
...
class KeyboardListener:
    def __init__(self) -> None:
        # ponytail: stdlib asyncio.Queue instead of custom aioqueue — fewer deps, same API
    ...
    async def start(self) -> None:
        ...
        self._listener = threading.Thread(
            ...
            name="coderai-keyboard-listener",
            daemon=True,
        )
...
def _listen_for_keyboard_unix(
    ...
    if sys.platform == "win32":
        raise RuntimeError("Unix keyboard listener requires non-Windows")
    import termios
...
def _listen_for_keyboard_windows(
    ...
    if sys.platform != "win32":
        raise RuntimeError("Windows keyboard listener requires Windows")
    import msvcrt
...
_ARROW_KEY_MAP: dict[bytes, KeyEvent] = {
    ...
    b"\x1bOA": KeyEvent.UP,
    b"\x1bOB": KeyEvent.DOWN,
    b"\x1bOC": KeyEvent.RIGHT,
    b"\x1bOD": KeyEvent.LEFT,
}
