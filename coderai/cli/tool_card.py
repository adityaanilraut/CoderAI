"""Formatted Tool Result Cards for tool executions matching DeepSeek Harness UI models."""
...
def _render_bash_card(
    ...
    header_text = f"    ↳ [bold cyan]$ {cmd}[/] [{status_style}]({status_text})[/]" if cmd else f"    ↳ [{status_style}]Shell Output ({status_text})[/]"
    ...
    if console is not None and _RICH:
        ...
        if error_text and error_text.strip():
            content_lines.extend([f"[bold red]Error:[/] {l}" for l in error_text.strip().splitlines()])
        ...
        if content_lines:
            ...
            if len(content_lines) > limit:
                console.print(f"      [dim]... ({len(content_lines) - limit} more lines truncated)[/]")
    else:
        plain_header = f"    ↳ $ {cmd} ({status_text})" if cmd else f"    ↳ Shell Output ({status_text})"
        print(plain_header)
        if output_text and output_text.strip():
...
def _render_search_card(console: Any, output_text: str | None, metadata: dict[str, Any]) -> None:
    ...
    if console is not None and _RICH:
        title = f"    ↳ [bold cyan]Web Search:[/] [bold yellow]\"{query_title}\"[/]" if query_title else "    ↳ [bold cyan]Web Search Results[/]"
        console.print(title)
        for idx, src in enumerate(sources[:6], 1):
            ...
            console.print(f"      [bold cyan]{idx}.[/] [bold]{s_title}[/]{date_str} [dim]•[/] [dim cyan]{s_url}[/]")
            if snippet:
...
def _render_fetch_card(
    console: Any, output_text: str | None, error_text: str | None, metadata: dict[str, Any], ok: bool
) -> None:
    ...
    bytes_count = metadata.get("bytes") or metadata.get("content_length") or (len(output_text.encode("utf-8")) if output_text else 0)
    ...
    if console is not None and _RICH:
        console.print(f"    ↳ [bold cyan]WebFetch[/] [{status_style}][{status_code}][/] [dim]({size_str})[/] • [dim]{url}[/]")
        if output_text and output_text.strip():
...
def _render_read_card(console: Any, file_path: str, metadata: dict[str, Any], output_text: str | None) -> None:
    ...
    if console is not None and _RICH:
        ...
        if output_text and output_text.strip():
            ...
            if len(lines) > display_limit:
                console.print(f"      [dim]... ({len(lines) - display_limit} more lines truncated)[/]")
    else:
        print(f"    ↳ Read: {file_path} ({lines_cnt or len(output_text.splitlines()) if output_text else 0} lines)")
...
def _render_search_grep_card(
    ...
    matches_count = metadata.get("matches_count") or (len(output_text.splitlines()) if output_text else 0)
...
def _render_lsp_card(console: Any, output_text: str | None, metadata: dict[str, Any], ok: bool) -> None:
    ...
    if console is not None and _RICH:
        ...
        console.print(f"    ↳ [bold yellow]LSP Diagnostics:[/] [bold cyan]{file_path}[/] [dim]({diag_count} issues)[/]")
        if diagnostics:
...
def _render_subagent_card(console: Any, output_text: str | None, metadata: dict[str, Any], ok: bool) -> None:
...
def render_tool_card(console: Any | None, message: SessionMessage) -> None:
    ...
    if console is not None and _RICH:
        ...
        if metadata:
            ...
            if name in ("UpdatePlan", "update_plan", "write_plan") and isinstance(plan_text, str) and plan_text.strip():
        ...
        if metadata:
            ...
            plan_text = metadata.get("plan")
            if name in ("UpdatePlan", "update_plan") and isinstance(plan_text, str) and plan_text.strip():
