...
def _persist_image(data_url: str) -> str | None:
    """Write an ACP image data-URL to a temp file so Stack A's read tool can load it."""
    try:
        header, _, payload = data_url.partition(",")
        if not payload:
            return None
        ext = ".png"
        for mime, suffix in _IMAGE_EXT_BY_MIME.items():
            if mime in header:
                ext = suffix
                break
        raw = base64.b64decode(payload)
        fd, name = tempfile.mkstemp(prefix="coderai-acp-", suffix=ext)
        with open(fd, "wb") as handle:
            handle.write(raw)
        return name
    except Exception:
        logger.warning("Failed to persist ACP image part")
        return None
...
def build_prompt(user_input: list[ContentPart]) -> str:
    ...
    for part in user_input:
        if isinstance(part, TextPart):
            ...
            path = _persist_image(part.image_url.url)
            if path:
                chunks.append(f"[Image attached and saved to {path} — use the read tool to view it.]")
            else:
...
class SessionManagerEngine:
    ...
    async def run(
        ...
        try:
            async for message in self._drive(self._start_turn(build_prompt(user_input)), ui_side):
                yield message
    ...
    async def _start_turn(self, text: str) -> None:
        if self._engine_session_id is None:
            self._engine_session_id = await self._manager.create_session(
                text or "", plan_mode=self._plan_mode, skills=self._skills
            )
        else:
            await self._manager.reply_session(self._engine_session_id, text)
...
__all__ = ["SessionManagerEngine", "build_prompt"]
