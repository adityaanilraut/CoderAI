# Ported from kimi_cli/acp/tools.py - kimi structure.
"""ACP-side tool adapters and tool replacement for ACP clients."""
...
from coderai.tools.shell import Params as ShellParams
from coderai.tools.shell import Shell
from coderai.tools.utils import ToolResultBuilder
from coderai.wire.types import DisplayBlock
...
def replace_tools(
    ...
    if client_capabilities.terminal and (shell_tool := toolset.find(Shell)):
        approval = getattr(runtime, "approval", None)
        if approval is None:
            approval = Approval()
        toolset.add(
            Terminal(
...
class Terminal(CallableTool2[ShellParams]):
    ...
    async def __call__(self, params: ShellParams) -> ToolReturnValue:
        from coderai.acp.session import get_current_acp_tool_call_id_or_none
        ...
        try:
            ...
            if acp_tool_call_id is not None:
...
__all__ = ["HideOutputDisplayBlock", "Terminal", "replace_tools"]
