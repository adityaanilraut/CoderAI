...
asyncio's exception handler, record a ``crash`` event, then delegate to the
original handler so the traceback still gets printed.

Privacy: only the exception *class name* is recorded. No message, no stack
trace — those can contain file paths, user input fragments, or URLs.

Adapted onto CoderAI's error log (``coderai.utils.logging``): crashes append
a class-name-only line to the shared error log. If a ``track()`` sink ever
lands in ``coderai.telemetry``, events flow there too (lazy, optional).
"""
...
def _record_crash(*, error_type: str, source: str) -> None:
    """Record one crash event. Telemetry failures must never mask the crash."""
    try:
        from coderai.telemetry import track  # type: ignore[attr-defined]
        ...
        pass
    try:
        from coderai.utils.logging import ERROR_LOG_PATH, LOG_DIR

        LOG_DIR.mkdir(parents=True, exist_ok=True)
        with open(ERROR_LOG_PATH, "a", encoding="utf-8") as f:
            f.write(f"[Crash:{error_type}] where={_phase} source={source}\n")
    except Exception:
        pass
...
def _excepthook(
    ...
    if not _should_ignore_for_excepthook(exc_type):
        ...
        try:
            _record_crash(error_type=exc_type.__name__, source="excepthook")
        except Exception:
            pass
...
def _asyncio_handler(
    ...
    if exc is not None and not isinstance(exc, asyncio.CancelledError):
        try:
            _record_crash(error_type=type(exc).__name__, source="asyncio_task")
        except Exception:
            pass
