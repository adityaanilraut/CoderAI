...
class AsyncTransport:
    ...
    async def send(self, events: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.warning(
                "Telemetry payload schema violation, dropping %d events: %s",
                len(events),
                exc,
            )
            return
            ...
            for attempt_idx in range(len(self._retry_backoffs) + 1):
                try:
                    ...
                    if attempt_idx >= len(self._retry_backoffs):
                        logger.debug(
                            "Telemetry send transient failure after %d attempts: %s",
                            attempt_idx + 1,
                            exc,
                        )
                        break
                    ...
                    if resp.status == 401 and token:
                        ...
                        async with session.post(
                            ...
                            if retry_resp.status >= 500 or retry_resp.status == 429:
                                ...
                                logger.debug(
                                    "Anonymous retry got client error HTTP %d, dropping",
                                    retry_resp.status,
                                )
                                return
                            return
                        ...
                        logger.debug(
                            "Telemetry got client error HTTP %d, dropping",
                            resp.status,
                        )
                        return
        ...
        for path in failed_files:
            ...
            try:
                if now - path.stat().st_mtime > DISK_EVENT_MAX_AGE_S:
                    logger.debug("Removing expired telemetry file: %s", path)
            ...
            try:
                ...
                path.unlink(missing_ok=True)
                logger.debug(
                    "Retried %d telemetry events from %s",
                    len(events),
                    path,
                )
            except _TransientError:
                # Still failing — leave file for next startup
                logger.debug("Retry of %s failed, will try again later", path)
            except json.JSONDecodeError:
                # Corrupted file — delete it
                logger.debug("Removing corrupted telemetry file: %s", path)
                path.unlink(missing_ok=True)
            except Exception:
                # Unexpected error — leave file for next startup
                logger.debug("Unexpected error retrying %s, will try again later", path)
