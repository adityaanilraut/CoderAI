        ...
        for st in body:
            if isinstance(st, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                ...
                if isinstance(st, ast.ClassDef):
                    ...
                    ext = any(ast.unparse(b) not in ("object",) for b in st.bases)
                    ...
                    refs = names_in(st)
                    is_root = (registering or (st.name.startswith("__") and st.name.endswith("__"))) and f not in EXCLUDE_FILES
...
json.dump(out, open("/tmp/rv/audit2/transdead.json", "w"), indent=0)
