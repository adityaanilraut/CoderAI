        ...
        if isinstance(_n, (ast.Import, ast.ImportFrom)):
            for a in _n.names:
                if a.asname and a.asname != a.name:
                    ALIAS.setdefault(a.asname, set()).add(a.name.split(".")[-1])
by_name = {}
...
def _expand(n, acc):
    for x in ALIAS.get(n, ()):
        if x not in acc:
            acc.add(x); _expand(x, acc)
_extra=set()
for n in list(live_names): _expand(n,_extra)
live_names |= _extra
...
while queue:
    ...
    if i in live:
        continue
    live.add(i)
    for n0 in defs[i]["refs"]:
        cand={n0}; _expand(n0,cand)
        for n in cand:
...
json.dump(out, open("/tmp/rv/audit2/transdead3.json", "w"), indent=0)
