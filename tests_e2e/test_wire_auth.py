...
Uses the ``error:`` scripted-echo DSL directive to simulate provider-level
HTTP errors (401, 429, connection, timeout) and verifies they surface
correctly through the wire protocol as provider errors (code -32003).
"""
...
def test_provider_connection_error_surfaces(tmp_path) -> None:
    """A simulated connection error is retried; the turn recovers afterwards.

    CoderAI retries transient provider errors, so the scripted error is
    consumed and the follow-up script completes the turn instead of the
    turn collapsing into a wire error.
    """
    config_path = write_scripted_config(
        tmp_path,
        ["error: connection Server unreachable", "text: recovered"],
    )
    work_dir = make_work_dir(tmp_path)
    home_dir = make_home_dir(tmp_path)
    ...
    try:
        ...
        resp, _msgs = collect_until_response(wire, "prompt-1")
        result = normalize_response(resp)
        assert result.get("result", {}).get("status") == "finished"
    finally:
        wire.close()


def test_provider_500_surfaces_error(tmp_path) -> None:
    """A 500 from the provider is retried; the turn recovers afterwards."""
    config_path = write_scripted_config(
        tmp_path,
        ["error: 500 Internal server error", "text: recovered"],
    )
    work_dir = make_work_dir(tmp_path)
    home_dir = make_home_dir(tmp_path)
