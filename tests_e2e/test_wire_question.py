...
def _question_request_handler(answers: dict[str, str]):
    ...
    def handler(msg: dict[str, Any]) -> dict[str, Any]:
        ...
        from tests_e2e.wire_helpers import build_approval_response
...
def test_ask_user_tool_hidden_when_question_not_supported(tmp_path) -> None:
    ...
    try:
        ...
        # The suppressed AskUserQuestion call must not stall the turn: the
        # wire stream advances past the blocked step and the turn finishes.
        # (CoderAI forwards TurnBegin/StepBegin progress markers; per-tool
        # ToolResult payloads stay inside the session, not on the wire.)
        step_begins = [m for m in summary if m.get("type") == "StepBegin"]
        assert len(step_begins) >= 2, (
            "expected the turn to advance past the suppressed question step"
        )
    finally:
        wire.close()
