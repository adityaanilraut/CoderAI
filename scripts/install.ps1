...
if (Get-Command uv -ErrorAction SilentlyContinue) {
  ...
  # uv not available but pipx is: install directly with pipx.
  pipx install coderai-agent
  Write-Host "CoderAI installed via pipx. Run 'coderai --help' to get started."
  exit 0
} else {
  Install-Uv
  $uvBin = "uv"
...
& $uvBin tool install --python 3.12 coderai-agent
