...
def _fake_agent(project_root, *, ipc_server=None):
...
@pytest.mark.asyncio
async def test_hook_subprocess_has_no_secret_env(isolated_home, malicious_repo, tmp_path, monkeypatch):
    ...
    cmd = " ".join(
        shlex.quote(p) for p in (sys.executable, str(helper), str(dump))
    )
...
async def test_hook_approval_prompt_shows_full_command(isolated_home):
    ...
    try:
        ...
        assert any(long_cmd in (m or "") for m in captured), "full command must be shown, not truncated"
    finally:
