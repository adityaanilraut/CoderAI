...
class Agent:
    ...
    async def process_message(self, user_message: str) -> Dict[str, Any]:
        ...
        # Include CANCELLED so the next user turn gets a fresh AgentInfo with a
        # cleared cancel event (request_cancel() sets an asyncio.Event forever).
        ...
        async def _run_hooks(tool_name: str, hook_type: str, arguments: dict) -> list:
            ...
            try:
                ...
                # Pass the full payload as JSON and also expose each argument
                # under a stable key-based name. Positional ``CODERAI_ARG_<i>``
                # is preserved for backward compatibility but deprecated —
                # LLM tool calls are keyword-style and the insertion order
                # isn't guaranteed.
                try:
                ...
                for i, (arg_key, arg_val) in enumerate(arguments.items()):
                    safe_key = (
                        "".join(
                            ...
                            for c in str(arg_key)
                        ).upper()
                    )
                    env[f"CODERAI_ARG_{safe_key}"] = str(arg_val)
                    env[f"CODERAI_ARG_{i}"] = str(arg_val)  # deprecated
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                # Run all read-only tools in parallel. Race each call against
                # the cancel event so a ``/stop`` during a long read is honored.
                cancel_event = (
                    self.tracker_info._cancel_event if self.tracker_info else None
                )

                async def _run_with_cancel(pc):
                    coro = _execute_single_tool(pc)
                    ...
                    if task in done:
                        cancel_wait.cancel()
                        return task.result()
                    # Cancellation requested — abort the tool.
                    task.cancel()
                    try:
                        await task
                ...
                for idx in mut_indices:
                    if cancel_event is not None and cancel_event.is_set():
                        ...
                        continue
                    t0 = _time.time()
                    results[idx] = await _run_with_cancel(parsed_calls[idx])
    ...
    def _summarize_tool_result(self, result) -> Dict[str, Any]:
        ...
        # Always run recursive truncation — it's idempotent for values that
        # already fit the per-string / per-list thresholds, and required for
        # over-long lists even when the total JSON size is small.
        per_string_max = self.config.max_tool_output // 2
        summarized = truncate_recursive(result, per_string_max)

        # ``max_tool_output`` names a *per-string* budget (strings larger
        # than ``max_tool_output // 2`` are split). A well-structured tool
        # result can still legitimately have several such strings plus
        # envelope keys, so we cap the *overall* JSON at 2× the per-string
        # budget. Outputs beyond that are clearly pathological and are
        # replaced with a terse diagnostic so the LLM doesn't repeat the
        # same expensive call.
    ...
    async def _stream_response(
        ...
        Falls back to a non-streaming ``chat`` call when no ``streaming_handler``
        is attached (e.g. programmatic use outside the Ink UI).
        """
        if self.streaming_handler is None:
            raw = await self.provider.chat(messages, tools=tools)
